...
**Short version:** if the harness speaks the OpenAI chat-completions protocol, KeylessAI is a one-line `baseURL` swap. If it speaks Anthropic's Messages API (Claude Code, some custom Anthropic-style harnesses), run a LiteLLM proxy in the middle to translate.

## Harness compatibility matrix

| Harness | Protocol | Works? | How |
|---|---|---|---|
| **Codex CLI** (OpenAI) | OpenAI | direct | `OPENAI_API_BASE=https://text.pollinations.ai OPENAI_API_KEY=not-needed codex ...` |
| **Aider** | OpenAI-compat (LiteLLM) | direct | `aider --model openai/openai-fast --openai-api-base https://text.pollinations.ai --openai-api-key not-needed` |
...
| **OpenHands (fka OpenDevin)** | OpenAI via LiteLLM | direct | Set `LLM_BASE_URL=https://text.pollinations.ai`, `LLM_API_KEY=not-needed`, `LLM_MODEL=openai/openai-fast` |
| **SWE-agent** | OpenAI or Anthropic | direct (OpenAI mode) | `--model openai/openai-fast --openai-api-base ...` |
| **Claude Code** | Anthropic Messages | **via LiteLLM bridge** | see below |
| **Cursor** | proprietary | partial | Cursor's "custom model" feature accepts an OpenAI endpoint. Settings &rarr; Models &rarr; Override OpenAI Base URL &rarr; `https://text.pollinations.ai` |
| **LangGraph / LangChain agents** | OpenAI | direct | `ChatOpenAI(base_url=..., api_key="not-needed")` |
| **CrewAI** | OpenAI via LiteLLM | direct | `OPENAI_API_BASE`, `OPENAI_MODEL_NAME=openai-fast` |
| **AutoGen / AG2** | OpenAI | direct | `config_list = [{"model": "openai-fast", "base_url": "https://text.pollinations.ai", "api_key": "not-needed"}]` |
...
Claude Code sends Anthropic-format Messages API calls to whatever `ANTHROPIC_BASE_URL` points at. Pollinations doesn't speak Anthropic format. [LiteLLM](https://github.com/BerriAI/litellm) is a protocol translator that accepts either format and forwards in the other &mdash; perfect middle layer.
...
Claude Code now sends Anthropic-shaped requests to your local LiteLLM, which translates them into OpenAI-shaped requests to Pollinations, which returns free GPT-OSS-20B completions, which LiteLLM translates back to Anthropic shape. Claude Code sees a normal Anthropic response and works.
...
export OPENAI_API_BASE=https://text.pollinations.ai OPENAI_API_KEY=not-needed OPENAI_MODEL=openai-fast
